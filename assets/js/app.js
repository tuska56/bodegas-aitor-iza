(function () {
  ...
  const cartStepLines = document.getElementById("cartStepLines");
  ...
  function renderCart() {
    ...
    cartNextBtn.disabled = items.length === 0;

    if (step === "lines") {
      cartWhatsBtn.hidden = true;
    }
  }
  ...
  /* ---------------- step: lines -> form -> whatsapp ---------------- */
  function goToStep(target) {
    step = target;
    const isForm = target === "form";
    cartStepLines.hidden = isForm;
    cartStepForm.hidden = !isForm;
    cartNextBtn.hidden = isForm;
    cartBackBtn.hidden = !isForm;
    cartWhatsBtn.hidden = !isForm;
    if (isForm) updateWhatsAppLink();
  }
  ...
  cartBackBtn.addEventListener("click", () => goToStep("lines"));

  custDelivery.addEventListener("change", () => {
    addressField.style.display = custDelivery.value === "Envío a domicilio" ? "flex" : "none";
    updateWhatsAppLink();
  });
  ...
  function buildOrderMessage() {
    ...
    const name = custName.value.trim() || "(sin indicar)";
    const phone = custPhone.value.trim() || "(sin indicar)";
    const delivery = custDelivery.value;
    ...
    if (delivery === "Envío a domicilio" && address) msg += `Dirección: ${address}\n`;
    if (notes) msg += `Notas: ${notes}\n`;
    msg += `\n¿Podéis confirmarme disponibilidad y precio final? Gracias.`;
    return msg;
  }
  ...
  cartWhatsBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      goToStep("lines");
      closeCart();
      [custName, custPhone, custAddress, custNotes].forEach((el) => (el.value = ""));
    }, 600);
  });
